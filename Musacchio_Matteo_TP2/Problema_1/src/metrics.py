...
def roc_auc_score(y_true, y_proba):
    ...
    return np.trapz(tpr_list, fpr_list)  # Integración numérica para AUC
...
def average_precision_score(y_true, y_proba):
    ...
    return np.trapz(precision_list, recall_list)  # Integración numérica para AUC-PR
...
def plot_roc_curve(y_true, y_proba):
    ...
    plt.plot(fpr_list, tpr_list, label=f"AUC-ROC = {roc_auc_score(y_true, y_proba):.2f}", color="blue")
...
def plot_precision_recall_curve(y_true, y_proba):
    ...
    plt.figure(figsize=(6, 6))
    plt.plot(recall_list, precision_list, label=f"AUC-PR = {average_precision_score(y_true, y_proba):.2f}", color="red")
    plt.xlabel("Recall")
